...
export default function Investigations() {
  ...
  const groupedByEmployeeFromCheckups = employees
    .map((employee) => {
      ...
      // Filter checkups based on status filter
      const filteredCheckups = employeeCheckups.filter((c: any) => {
      ...
      return {
        ...
        investigationNames: [...new Set(filteredCheckups.map((c: any) => c.investigation_name).filter(Boolean))].join(", "),
      };
    })
    .filter((item) => {
      ...
      return item.checkups.length > 0 && matchesDepartment && matchesGroup && matchesSearch;
    });
